...
const ListsContainer = ({ currentUser, chosenList, onFormSubmit }) => {
  ...
  const readingListItems = currentUser.reading.map ((book) => {
    return <ListItem key={book.id} thumbnail={book.thumbnail} title={book.title} authors={book.authors} averageRating ={book.average_rating} onFormSubmit={onFormSubmit} currentUser={currentUser}/>
  })
